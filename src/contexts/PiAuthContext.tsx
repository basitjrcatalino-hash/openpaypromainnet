...
export function PiAuthProvider({ children }: { children: ReactNode }) {
  const [session, setSession] = useState<PiAuthSession | null>(() =>
    typeof window === "undefined" ? null : loadPiAuthSession(),
  );
  const [sdkReady, setSdkReady] = useState(false);
  const [loading, setLoading] = useState(true);
  const [authError, setAuthError] = useState<string | null>(null);
  const autoLoginAttempted = useRef(false);
  const inPiBrowser = typeof window === "undefined" ? false : isPiBrowser();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (stored && inPiBrowser && ready) {
        try { await refreshSession(); } catch { /* handled inside */ }
      } else if (stored) {
        setSession(stored);
      }
