...
function OpenTokenHome() {
  ...
  const { data: tokens = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .limit(200);
      if (error) {
        // Fallback if migration columns missing
  ...
  const list = useMemo(() => {
    ...
    else if (filter === "new") l = [...l].sort((a, b) => +new Date(b.created_at) - +new Date(a.created_at));
  ...
  return (
    <div className="animate-page-in space-y-6">
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">Trending now</h2>
            ...
            {topMcap.map((t: any, i: number) => (
              <li key={t.id}>
                ...
                >
                  ...
                  <div className="text-right text-sm tabular-nums">{formatUSD(t.market_cap, { compact: true })}</div>
                </Link>
              </li>
            ))}
          </ul>
