...
async function authorize(request: Request): Promise<true | Response> {
  ...
  return data ? true : json({ error: "Invalid or inactive API key" }, 401);
}

export const Route = createFileRoute("/api/public/ledger/stats")({
  server: {
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        const { data: latest } = await supabaseAdmin
          .from("ledger_entries").select("sequence, occurred_at")
