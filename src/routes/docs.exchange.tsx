...
export const Route = createFileRoute("/docs/exchange")({
  head: () => ({
    meta: [
      ...
      { property: "og:url", content: `${DOCS_BASE}/docs/exchange` },
    ],
    links: [{ rel: "canonical", href: `${DOCS_BASE}/docs/exchange` }],
  }),
  component: ExchangeDocsPage,
});

const PARTNER_BASE = PARTNER_API;
const PRO_HOST = DOCS_BASE;
...
function ExchangeDocsPage() {
  return (
    <DocsShell
      ...
      description="List OpenPay like another network: deposit and withdraw OUSD via Partner Transfer API, credit Pro wallets with inbound, and reconcile on the public ledger."
      pathname="/docs/exchange"
      eyebrow="Core guides"
    ...
    >
      ...
      <DocsSection id="network" eyebrow="Step 1" title="How OpenPay maps to a listed network">
      ...
      <DocsSection id="bases" eyebrow="Step 2" title="API base URLs">
        <Card className="space-y-4 rounded-3xl border-border bg-card p-5 text-sm shadow-none">
          <div className="space-y-2">
            ...
            <DocsCode>{PARTNER_BASE}</DocsCode>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-2">
              ...
              <Button
                ...
                variant="ghost"
                className="h-7 rounded-full"
                onClick={() => copy(`${PRO_HOST}/api/public/openpay/inbound`, "Copied")}
              >
                <Copy className="h-3.5 w-3.5" />
              </Button>
            </div>
            <DocsCode>{`POST ${PRO_HOST}/api/public/openpay/inbound`}</DocsCode>
          </div>
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-2">
            ...
            <DocsCode>{`${PRO_HOST}/api/public/ledger`}</DocsCode>
          </div>
      ...
      <DocsSection id="deposit" eyebrow="Step 3" title="Deposit (user → your exchange)">
        <Card className="space-y-4 rounded-3xl border-border bg-card p-5 text-sm text-muted-foreground shadow-none">
          <div className="flex items-start gap-3">
            ...
            <p className="text-foreground">
              Recommended: create a <strong>charge</strong>, send the user to OpenPay checkout, poll
              until <code className="rounded bg-muted px-1">paid</code>, then credit their exchange
              balance (idempotent on charge id / reference).
            </p>
          </div>
          <DocsCode>{`curl -X POST "${PARTNER_BASE}/charges" \\
  ...
  -d '{
    ...
    "cancel_url": "https://your.exchange/deposits/openpay/cancel"
          ...
          <p>
            ...
            <code className="rounded bg-muted px-1 text-foreground">OP…</code> hot wallet, or credit
            Pro users via{" "}
            <Link
              to="/docs/openpay"
              hash="openpay-to-pro"
      ...
      <DocsSection id="withdraw" eyebrow="Step 4" title="Withdraw (your exchange → user)">
        <Card className="space-y-4 rounded-3xl border-border bg-card p-5 text-sm text-muted-foreground shadow-none">
          <div className="flex items-start gap-3">
          ...
          <DocsCode>{`curl -X POST "${PARTNER_BASE}/transfers" \\
  -H "Authorization: Bearer opk_live_YOUR_KEY" \\
  -H "Content-Type: application/json" \\
  ...
  -d '{
    ...
    "note": "Withdraw from YourExchange #9001"
      ...
      <DocsSection id="swap" eyebrow="Step 5" title="Swap / convert">
        <Card className="space-y-3 rounded-3xl border-border bg-card p-5 text-sm text-muted-foreground shadow-none">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2 text-foreground">
              ...
              <p className="text-muted-foreground">
                There is no public partner endpoint that runs OpenDEX swaps for arbitrary users yet
                — use transfers + your order book, or deep-link to Pro.
              </p>
            </div>
          </div>
        </Card>
      ...
      <DocsSection id="ledger" eyebrow="Step 6" title="Ledger & reconciliation">
        <Card className="space-y-3 rounded-3xl border-border bg-card p-5 text-sm text-muted-foreground shadow-none">
          <p className="text-foreground">
            Poll{" "}
      ...
      <DocsSection id="checklist" eyebrow="Step 7" title="Launch checklist">
