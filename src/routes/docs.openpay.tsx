...
const API = PARTNER_API;
const CONNECT = CONNECT_URL;

const DOCS_SPEECH = `
OpenPay Integration Docs. Connect auth, Pro sign-in methods, and payments.

This guide helps third-party apps add Connect with OpenPay using OAuth 2.0, integrate every OpenPay Pro wallet auth method — Solana, Phantom, Pi, WalletConnect, MetaMask Embedded, and Telegram — and accept OpenPay balance payments.
...
function OpenPayDocsPage() {
  return (
    <DocsShell
      title="Connect auth, Pro sign-in methods & payments"
      description="Add Connect with OpenPay (OAuth 2.0), integrate every OpenPay Pro wallet auth method (Solana, Phantom, Pi, WalletConnect, MetaMask Embedded, Telegram), and accept OpenPay balance payments."
    ...
    >
      <div className="flex flex-wrap gap-2">
        ...
        <Button
          ...
          onClick={() => copy(PARTNER_API, "API base copied")}
        >
      ...
      <nav className="grid gap-2 rounded-3xl border border-border bg-card p-4 text-sm sm:grid-cols-2">
        {[
          ...
          <a key={href} href={href} className="rounded-xl px-3 py-2 text-foreground hover:bg-muted">
            {label}
          </a>
        ))}
      </nav>
      ...
      <DocsSection id="exchange" eyebrow="Exchanges" title="List OUSD on OpenPay Network">
        <Card className="space-y-3 rounded-3xl border-border bg-card p-5 text-sm text-muted-foreground shadow-none">
          <p className="text-foreground">
            ...
            network id <code className="rounded bg-muted px-1">openpay</code>, Partner Transfer for
...
# Enable Google / X / Apple / GitHub / Discord / Facebook in developer.metamask.io`}</DocsCode>
...
# http://localhost:PORT (+ /auth/callback)`}</DocsCode>
...
// Phantom: PhantomContinueButton / Google·Apple via @phantom/react-sdk`}</DocsCode>
        ...
        <Card className="space-y-2 rounded-3xl border-border bg-card p-5 text-sm shadow-none">
          ...
          <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
            ...
            <li>
              Web3Auth: always check JWT <code className="text-foreground">aud</code> = your Client
              ID
            </li>
...
# Cancel return:  ?openpay_cancel=1`}</DocsCode>
      ...
      <DocsSection id="openpay-to-pro" eyebrow="Step 5" title="OpenPay → OpenPay Pro transfers">
        <Card className="space-y-3 rounded-3xl border-border bg-card p-5 text-sm text-muted-foreground shadow-none">
          <p className="text-foreground">
          ...
          <p>
            Note:{" "}
            <code className="rounded bg-muted px-1 text-foreground">pro_xfer:@alice:r_ref</code> or{" "}
            <code className="rounded bg-muted px-1 text-foreground">pro_xfer:0x…:r_ref</code> (Pro
            wallet address)
          </p>
          <DocsCode>{`# By @username
...
curl -X POST "https://openpaypromainnet.lovable.app/api/public/openpay/inbound" \\
  -H "Authorization: Bearer opk_live_YOUR_KEY" \\
  -H "Content-Type: application/json" \\
  -d '{"to":"0x7bf2…851a","amount":25,"openpay_tx_id":"TX2","note":"pro_xfer:0x7bf2…851a:r_2"}'`}</DocsCode>
          <p>
            Pro users:{" "}
      ...
      <DocsSection id="api" eyebrow="Step 6" title="API cheat sheet">
      ...
      <DocsSection id="ledger" eyebrow="Step 7" title="Public Ledger API">
        <Card className="space-y-3 rounded-3xl border-border bg-card p-5 text-sm text-muted-foreground shadow-none">
          <p className="text-foreground">
          ...
          <DocsCode>{`GET https://openpaypromainnet.lovable.app/api/public/ledger/entries?limit=50
x-api-key: YOUR_LEDGER_KEY`}</DocsCode>
      ...
      <DocsSection id="wc-pay" eyebrow="Step 8" title="WalletConnect Pay">
        <Card className="space-y-3 rounded-3xl border-border bg-card p-5 text-sm text-muted-foreground shadow-none">
          <p className="text-foreground">
      ...
      <DocsSection id="charges" eyebrow="Step 9" title="Charges polling (no partner webhooks)">
        <Card className="space-y-3 rounded-3xl border-border bg-card p-5 text-sm text-muted-foreground shadow-none">
          <p className="text-foreground">
            Partner payments are confirmed by <strong>polling</strong>{" "}
            <code className="rounded bg-muted px-1">GET /charges/:id</code> (or success URL return).
            There is no partner-facing payment webhook today.
          </p>
          <DocsCode>{`# After creating a charge
GET ${PARTNER_API}/charges/CHARGE_ID
Authorization: Bearer opk_live_YOUR_KEY

# Fulfill only when status is paid / confirmed`}</DocsCode>
          <p>
      ...
      <DocsSection id="nft" eyebrow="Step 10" title="OpenNFT mint (high level)">
        <Card className="space-y-3 rounded-3xl border-border bg-card p-5 text-sm text-muted-foreground shadow-none">
          <p className="text-foreground">
      ...
      <DocsSection id="errors" eyebrow="Step 11" title="Errors & launch checklist">
        <Card className="space-y-2 rounded-3xl border-border bg-card p-5 text-sm shadow-none">
          <p>
          ...
          <p>
            <strong>Scopes</strong> — Connect uses{" "}
      ...
      <footer className="border-t border-border pt-6 text-xs text-muted-foreground">
        <p>
          Integration markdown: <code className="text-foreground">docs/OPENPAY_INTEGRATION.md</code>{" "}
          · Auth methods: <code className="text-foreground">docs/OPENPAY_PRO_AUTH.md</code> ·
          Partner portal:{" "}
          <a
            href={PARTNER_PORTAL}
            target="_blank"
