...
function Pill({
  ...
  const className = primary
    ? "inline-flex items-center gap-1.5 rounded-full bg-[var(--primary)] px-4 py-2.5 text-sm font-bold text-[var(--primary-foreground)] shadow-sm transition hover:opacity-90 md:text-base"
    : "inline-flex items-center gap-1.5 rounded-full border border-[var(--border)] bg-[var(--card)] px-4 py-2.5 text-sm font-semibold text-[var(--foreground)] transition hover:border-[var(--primary)] md:text-base";
  if (href) {
    ...
    return (
      <a
        ...
        {...(external && href.startsWith("http")
          ...
          : {})}
        {...(href.startsWith("/api/") ? { target: "_blank", rel: "noreferrer" } : {})}
      >
        {children}
      </a>
    );
    ...
    >
      ...
      <DocsCard>
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          <div className="rounded-2xl bg-[var(--muted)] px-3 py-2.5">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-0.5 block break-all text-sm font-semibold text-[var(--foreground)] underline-offset-2 hover:underline"
            >
              https://openpy.space
            </a>
          </div>
          <div className="rounded-2xl bg-[var(--muted)] px-3 py-2.5">
            ...
            <button
              type="button"
              className="mt-0.5 block w-full break-all text-left text-sm font-semibold text-[var(--foreground)]"
        ...
        <ol className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
          {[
            ...
            <li
              key={step}
              className="rounded-2xl border border-[var(--border)] bg-[var(--background)] px-3 py-2.5"
            >
              <div className="flex items-center gap-2">
                <span className="grid h-6 w-6 place-items-center rounded-full bg-[var(--primary)] text-[10px] font-bold text-[var(--primary-foreground)]">
                  {step.replace("Step ", "")}
                </span>
                <div>
      ...
      <nav className="grid gap-1.5 rounded-3xl border border-[var(--border)] bg-[var(--card)] p-4 text-sm sm:grid-cols-2">
        {TOC.map(([href, label]) => (
          <a
            key={href}
            href={href}
            className="rounded-xl px-3 py-2 font-medium text-[var(--foreground)] transition hover:bg-[var(--muted)]"
          >
            {label}
          </a>
        ))}
      ...
      <DocsSection id="partner-portal" eyebrow="Portal" title="OpenPay Partner API portal">
        <DocsCard className="space-y-3 text-sm text-[var(--muted-foreground)]">
          ...
          <ul className="list-disc space-y-1.5 pl-5">
            ...
            <li>
              ...
              <code className="rounded bg-[var(--muted)] px-1 text-[var(--foreground)]">
            ...
            <li>
              Never put <code className="rounded bg-[var(--muted)] px-1 text-[var(--foreground)]">opk_</code>{" "}
      ...
      <DocsSection id="exchange" eyebrow="Exchanges" title="List OUSD on OpenPay Network">
        <DocsCard className="space-y-3 text-sm text-[var(--muted-foreground)]">
          <p className="text-[var(--foreground)]">
            ...
            <code className="rounded bg-[var(--muted)] px-1">openpay</code>, Partner Transfer for
      ...
      <DocsSection id="setup" eyebrow="Step 1" title="Create a partner app">
        <DocsCard className="space-y-3 text-sm text-[var(--muted-foreground)]">
          <ol className="list-decimal space-y-2 pl-5 text-[var(--foreground)]">
            <li>
              Open{" "}
              <a
                href={PARTNER_PORTAL}
            ...
            <li>
              Copy the <code className="rounded bg-[var(--muted)] px-1">opk_live_…</code> API key
              immediately (shown once). Save the Client ID (UUID).
            </li>
            <li>
              Enter only your domain (e.g.{" "}
              <code className="rounded bg-[var(--muted)] px-1">www.yourapp.com</code>) and click{" "}
...
OPENPAY_REDIRECT_URI="https://yourapp.com/openpay/callback"`}</DocsCode>
      ...
      <DocsSection id="connect" eyebrow="Step 2" title="Connect with OpenPay">
        <DocsCallout>
          Authorization Code flow. Scopes: <code className="text-[var(--foreground)]">profile</code>,{" "}
          <code className="text-[var(--foreground)]">balance</code>. User lands on Authorize, signs
          in, Allow → your callback receives <code className="text-[var(--foreground)]">opc_…</code>.
        </DocsCallout>

        <p className="text-sm font-semibold text-[var(--foreground)]">Authorize URL</p>
  ...
  &state=RANDOM_CSRF_TOKEN`}</DocsCode>
        ...
        <p className="text-sm font-semibold text-[var(--foreground)]">Drop-in Connect button</p>
  ...
  Connect with OpenPay
        ...
        <p className="text-sm font-semibold text-[var(--foreground)]">Callback</p>
        <DocsCode>{`# Success
...
# Verify state matches what you stored`}</DocsCode>
        ...
        <p className="text-sm font-semibold text-[var(--foreground)]">Exchange code (server)</p>
  ...
  -d '{
    ...
    "client_secret": "opk_live_YOUR_KEY"
        ...
        <p className="text-sm font-semibold text-[var(--foreground)]">User APIs (Bearer opa_live_…)</p>
  ...
  "scope": "profile balance"
      ...
      <DocsSection id="auth" eyebrow="Step 3" title="OpenPay Pro — all sign-in methods">
        <DocsCallout>
          ...
          <Link to="/authpi" className="font-semibold text-[var(--foreground)] underline-offset-2 hover:underline">
            /authpi
          </Link>
          . Full Markdown:{" "}
          <a
          ...
          · live page{" "}
          <Link to="/docs/auth" className="font-semibold text-[var(--foreground)] underline-offset-2 hover:underline">
            /docs/auth
          </Link>
          .
        </DocsCallout>
          ...
          {AUTH_METHODS.map((m) => (
            <DocsCard key={m.name} className="space-y-2 !p-4">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="rounded-full border border-[var(--border)] px-2 py-0.5 text-[10px] font-semibold text-[var(--muted-foreground)]">
                  /authpi
                </span>
              </div>
              ...
              <dl className="space-y-1 text-[11px]">
                <div>
                  ...
                  <dd className="break-all font-mono text-[var(--foreground)]">{m.endpoint}</dd>
                </div>
                <div>
                  <dt className="text-[var(--muted-foreground)]">Callback</dt>
                  <dd className="font-mono text-[var(--foreground)]">{m.callback}</dd>
                </div>
                <div>
                  <dt className="text-[var(--muted-foreground)]">Env</dt>
                  <dd className="text-[var(--foreground)]">{m.env}</dd>
                </div>
              </dl>
            </DocsCard>
          ))}
        ...
        <p className="text-sm font-semibold text-[var(--foreground)]">Shared server secrets</p>
...
SUPABASE_SERVICE_ROLE_KEY="eyJ..."   # server only — admin.createUser`}</DocsCode>
        ...
        <p className="text-sm font-semibold text-[var(--foreground)]">MetaMask Embedded (Web3Auth)</p>
...
WEB3AUTH_JWKS_URL="https://api-auth.web3auth.io/.well-known/jwks.json"`}</DocsCode>
        ...
        <p className="text-sm font-semibold text-[var(--foreground)]">Phantom Portal</p>
        <DocsCode>{`VITE_PHANTOM_APP_ID="your-app-id"
# Allowlist each origin + /auth/callback in Phantom Portal`}</DocsCode>

        <p className="text-sm font-semibold text-[var(--foreground)]">Client starters</p>
...
await startWalletConnectSignIn({ redirectTo: "/dashboard" })`}</DocsCode>
        ...
        <DocsCard className="space-y-2 text-sm">
          ...
          <ul className="list-disc space-y-1 pl-5 text-[var(--muted-foreground)]">
            <li>
              Never expose <code className="text-[var(--foreground)]">opk_</code>,{" "}
              <code className="text-[var(--foreground)]">wcp_</code>,{" "}
              <code className="text-[var(--foreground)]">WEB3AUTH_CLIENT_SECRET</code>, or service
              role in the browser
            </li>
            <li>
              Verify OAuth <code className="text-[var(--foreground)]">state</code> on every callback
            </li>
            <li>
              Web3Auth: always check JWT <code className="text-[var(--foreground)]">aud</code> = your
              Client ID
            </li>
      ...
      <DocsSection id="pay" eyebrow="Step 4" title="Accept OpenPay balance payments">
        <div className="flex items-start gap-3 rounded-3xl border border-[var(--border)] bg-[var(--card)] p-4">
          <Wallet className="mt-0.5 h-5 w-5 shrink-0 text-[var(--primary)]" />
          <p className="text-sm text-[var(--muted-foreground)]">
            Buyer pays from their OpenPay wallet. Funds credit your partner-app owner. Prefer
            PayButton <code className="text-[var(--foreground)]">/charges</code>; use{" "}
            <code className="text-[var(--foreground)]">/pay/@username</code> for hosted tag payments.
          </p>
        </div>

        <p className="text-sm font-semibold text-[var(--foreground)]">A · PayButton charge</p>
...
# → redirect buyer to checkout_url or ${PAY_HOST}/paybutton/CHARGE_ID`}</DocsCode>
        ...
        <p className="text-sm font-semibold text-[var(--foreground)]">Poll / cancel</p>
...
curl -H "Authorization: Bearer opk_live_YOUR_KEY" \\
  "${PARTNER_API}/charges?status=paid"`}</DocsCode>
        ...
        <p className="text-sm font-semibold text-[var(--foreground)]">B · Hosted pay tag</p>
...
# Cancel return:  ?openpay_cancel=1`}</DocsCode>
        ...
        <DocsCard className="text-sm text-[var(--muted-foreground)]">
          <p className="font-semibold text-[var(--foreground)]">Pay flow</p>
          <ol className="mt-2 list-decimal space-y-1 pl-5">
            <li>Open pay link → amount + note</li>
            <li>Pay → balance check → debit OUSD → thank-you</li>
            <li>
              Redirect to your <code className="text-[var(--foreground)]">success_url</code>
            </li>
            <li>Backend verifies <code className="text-[var(--foreground)]">paid</code>, then fulfills</li>
          </ol>
        </DocsCard>
      </DocsSection>
      ...
      <DocsSection id="transfers" eyebrow="Step 5" title="Payouts — Partner transfers">
        <DocsCallout>
          ...
          <code className="text-[var(--foreground)]">Idempotency-Key</code>. Prefer{" "}
          <code className="text-[var(--foreground)]">OP…</code> account numbers over{" "}
          <code className="text-[var(--foreground)]">@username</code>.
        </DocsCallout>
...
curl -H "Authorization: Bearer opk_live_YOUR_KEY" ${PARTNER_API}/balance`}</DocsCode>
  ...
  -d '{
    ...
    "from_username": "bob"
          ...
          <p>
            Idempotent on <code className="text-[var(--foreground)]">openpay_tx_id</code>. Pro users
            can also <strong className="text-[var(--foreground)]">Receive → Create OpenPay receive link</strong>.
            Product deep-link:{" "}
            <Link to="/topup" className="font-semibold text-[var(--foreground)] underline-offset-2 hover:underline">
              /topup
            </Link>
            .
          </p>
  ...
  return res.json();
      ...
      <DocsSection id="api" eyebrow="Step 8" title="API cheat sheet">
          ...
          <table className="w-full min-w-[32rem] text-left text-sm">
            ...
            <tbody className="divide-y divide-[var(--border)]">
              {[
                ...
                <tr key={String(p)}>
                  <td className="px-4 py-2.5 font-mono text-xs text-[var(--foreground)]">{m}</td>
                  <td className="px-4 py-2.5 font-mono text-xs text-[var(--foreground)]">{p}</td>
                  <td className="px-4 py-2.5 text-[var(--muted-foreground)]">{a}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <p className="text-xs text-[var(--muted-foreground)]">
          Base: <code className="break-all text-[var(--foreground)]">{PARTNER_API}</code>
          <br />
          Full reference:{" "}
          <Link to="/docs/api" className="font-semibold text-[var(--foreground)] underline-offset-2 hover:underline">
            /docs/api
          </Link>{" "}
          · OpenAPI:{" "}
          <a
      ...
      <DocsSection id="ledger" eyebrow="Step 9" title="Public Ledger API">
        <DocsCard className="space-y-3 text-sm text-[var(--muted-foreground)]">
          <p className="text-[var(--foreground)]">
            ...
            <code className="rounded bg-[var(--muted)] px-1 text-[var(--foreground)]">x-api-key</code>{" "}
            or <code className="rounded bg-[var(--muted)] px-1 text-[var(--foreground)]">Authorization: Bearer</code>.
          </p>
...
x-api-key: YOUR_LEDGER_KEY`}</DocsCode>
          <p>
            ...
            <Link to="/docs/ledger" className="font-semibold text-[var(--foreground)] underline-offset-2 hover:underline">
              /docs/ledger
            </Link>
            .
          </p>
      ...
      <DocsSection id="wc-pay" eyebrow="Step 10" title="WalletConnect Pay">
        <DocsCard className="space-y-3 text-sm text-[var(--muted-foreground)]">
          <p className="text-[var(--foreground)]">
            ...
            <code className="rounded bg-[var(--muted)] px-1">/scan</code> →{" "}
            <code className="rounded bg-[var(--muted)] px-1">/wc-pay</code>).
          </p>
          <ul className="list-disc space-y-1.5 pl-5">
          ...
          <p>
            This is a <strong className="text-[var(--foreground)]">payer</strong> integration inside
      ...
      <DocsSection id="charges" eyebrow="Step 11" title="Charges polling (no partner webhooks)">
        <DocsCard className="space-y-3 text-sm text-[var(--muted-foreground)]">
          <p className="text-[var(--foreground)]">
            ...
            <code className="rounded bg-[var(--muted)] px-1">GET /charges/:id</code>. There is no
...
# Fulfill only when status === "paid"`}</DocsCode>
          <p>
            ...
            <strong className="text-[var(--foreground)]">not</strong> exposed to third-party apps.
          </p>
        </DocsCard>
      </DocsSection>
      ...
      <DocsSection id="nft" eyebrow="Step 12" title="OpenNFT mint (high level)">
        <DocsCard className="space-y-3 text-sm text-[var(--muted-foreground)]">
          ...
          <ul className="list-disc space-y-1.5 pl-5">
            ...
            <li>
              Ledger type <code className="rounded bg-[var(--muted)] px-1 text-[var(--foreground)]">mint</code>{" "}
      ...
      <DocsSection id="errors" eyebrow="Step 13" title="Errors & launch checklist">
        <DocsCard className="space-y-2 text-sm">
          <p>
            <strong className="text-[var(--foreground)]">401 / invalid_client</strong> — bad or
            quoted <code className="rounded bg-[var(--muted)] px-1">opk_live_…</code> / wrong
            client_id
          </p>
          <p>
            <strong className="text-[var(--foreground)]">redirect_uri not registered</strong> — must
            match allowlist exactly
          </p>
          <p>
            <strong className="text-[var(--foreground)]">403</strong> — origin not whitelisted
          </p>
          <p>
            <strong className="text-[var(--foreground)]">400</strong> — validation, insufficient
            balance
          </p>
          <p>
            <strong className="text-[var(--foreground)]">Scopes</strong> — Connect uses{" "}
            <code className="rounded bg-[var(--muted)] px-1">profile</code> and{" "}
            <code className="rounded bg-[var(--muted)] px-1">balance</code>
          </p>
        </DocsCard>
        <ul className="space-y-2 text-sm">
          {[
            ...
            <li key={item} className="flex gap-2 text-[var(--foreground)]">
              <span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-[var(--primary)] text-[10px] font-bold text-[var(--primary-foreground)]">
                ✓
              </span>
              {item}
            </li>
      ...
      <DocsSection id="faq" eyebrow="FAQ" title="OpenPay Pro FAQ">
        <DocsCard className="space-y-4 text-sm">
          <div>
            <p className="font-semibold text-[var(--foreground)]">How do I connect OpenPay to Pro?</p>
          ...
          <div>
            <p className="font-semibold text-[var(--foreground)]">Where do trade fees go?</p>
            <p className="mt-1 text-[var(--muted-foreground)]">
              Platform fees credit the admin fee wallet — typically{" "}
              <code className="rounded bg-[var(--muted)] px-1">@openpay</code>.
            </p>
          </div>
          <div>
            <p className="font-semibold text-[var(--foreground)]">Is there a partner webhook?</p>
            <p className="mt-1 text-[var(--muted-foreground)]">
              Not yet — poll <code className="rounded bg-[var(--muted)] px-1">GET /charges/:id</code>{" "}
      ...
      <footer className="border-t border-[var(--border)] pt-6 text-xs text-[var(--muted-foreground)]">
        <p>
          Integration: <code className="text-[var(--foreground)]">docs/OPENPAY_INTEGRATION.md</code>{" "}
          · Auth: <code className="text-[var(--foreground)]">docs/OPENPAY_PRO_AUTH.md</code> · AI
          pack:{" "}
          <Link to="/docs/ai" className="font-semibold text-[var(--foreground)] underline-offset-2 hover:underline">
            /docs/ai
          </Link>{" "}
          · Partner portal:{" "}
          <a
